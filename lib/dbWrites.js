...
/** Aynı okul/paket/ödeme bilgisiyle yeni denetim; cevap yok, durum pending. */
export async function duplicateInspectionForReinspect(sourceInspectionId, inspectorId) {
  const src = await prisma.inspection.findUnique({
    where: { id: sourceInspectionId },
  })
  if (!src) throw httpError('Denetim bulunamadı', 404)
  if (src.inspectorId !== inspectorId) {
    throw httpError('Bu denetim size atanmamış', 403)
  }
  if (src.status !== 'completed') {
    throw httpError('Yeniden denetim yalnızca tamamlanmış denetimlerden oluşturulabilir', 400)
  }
  if (src.paymentId) {
    ...
    if (pay && pay.status !== 'completed') {
      throw httpError('Ödeme tamamlanmamış denetim kopyalanamaz', 400)
    }
  }
  ...
  return prisma.inspection.create({
    data: {
      ...
      inspectorId: src.inspectorId,
      status: 'pending',
      schoolContact: src.schoolContact || '',
      schoolPhone: src.schoolPhone || '',
      schoolEmail: src.schoolEmail || '',
